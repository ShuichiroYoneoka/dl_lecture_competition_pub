...
class EVFlowNet(nn.Module):
    ...
    def forward(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_size = flow3.size()[2:]  #高さと幅を取得 #[8, 2, 480, 640]

        # 各テンソルをリサイズ
        flow1_resized = F.interpolate(flow1, size=target_size, mode='bilinear', align_corners=False)
        flow2_resized = F.interpolate(flow2, size=target_size, mode='bilinear', align_corners=False)
        flow0_resized = F.interpolate(flow0, size=target_size, mode='bilinear', align_corners=False)

        flow = torch.cat((flow0_resized, flow1_resized, flow2_resized, flow3), dim=1) #[8, 8, 480, 640]
        if flow.size() == torch.Size([8, 8, 480, 640]):
            flow = flow.view(8, 4, 2, 480, 640) #平均を取ってチャネル数を2に減らした
            flow = flow.mean(dim=1)

        else:
            pass

        return flow #flowが[8, 8, 480, 640]のsizeでない可能性もある
